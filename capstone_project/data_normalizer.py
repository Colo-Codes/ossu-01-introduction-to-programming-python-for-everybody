...
normalised_sqlite_connection = sqlite3.connect('capstone_normalized.sqlite')
    ...
    for year in data_dictionary:
        ...
        for region in regions:
            if data_type == 'dwellings':
                ...
                averaged_number_of_dwellings = sum(number_of_dwellings) / len(number_of_dwellings)
                averaged_mean_price = sum(mean_prices) / len(mean_prices)
                averaged_values = (averaged_number_of_dwellings, averaged_mean_price)
                
                print(f"    Averaging {year}, region {region} -> dwellings: {averaged_number_of_dwellings}, price: {averaged_mean_price}")
                ...
                print(f"    Averaging {year}, region {region} -> ratio: {averaged_values}")
...
raw_cursor.execute('SELECT id, time_period, number_of_dwellings, mean_price, region_id FROM dwellings ORDER BY region_id, time_period')
...
for year in averaged_data_dictionary:
    ...
    for region in regions:
        ...
        print(f"    Inserting number of dwellings ({number_of_dwellings}) and mean price ({mean_price}) into database... YEAR: {year}, REGION: {region}")
        normalised_cursor.execute('''INSERT OR IGNORE INTO dwellings (time_period, number_of_dwellings, mean_price, region_id)
                        VALUES (?, ?, ?, ?)''', (year, number_of_dwellings, mean_price, region) )
...
raw_cursor.execute('SELECT id, time_period, ratio, region_id FROM employment_ratios ORDER BY region_id, time_period')
...
for year in averaged_data_dictionary:
    ...
    for region in regions:
        ...
        print(f"    Inserting ratio ({ratio}) into database... YEAR: {year}, REGION: {region}")
        normalised_cursor.execute('''INSERT OR IGNORE INTO employment_ratios (time_period, ratio, region_id)
                        VALUES (?, ?, ?)''', (year, ratio, region) )
...
raw_cursor.execute('SELECT id, time_period, value, region_id FROM migrations ORDER BY time_period, region_id')
...
for row_data in migrations_ratios_rows:
    ...
    value = row_data[2]
    region = row_data[3]
    print(f"    Inserting value ({value}) into database... YEAR: {year}, REGION: {region}")
    normalised_cursor.execute('''INSERT OR IGNORE INTO migrations (time_period, value, region_id)
                        VALUES (?, ?, ?)''', (year, value, region) )
...
for row_data in regions_rows:
    ...
    normalised_cursor.execute('''INSERT OR IGNORE INTO regions (id, name)
                        VALUES (?, ?)''', (id, name) )
